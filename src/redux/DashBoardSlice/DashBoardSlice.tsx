...
export const GetProFileDashAction = createAsyncThunk('DashBoardSlice/GetProFileAction', async (id: number) => {
    try {
...
export const DelectProfilePictureAction = createAsyncThunk(
    ...
    async (useId: number) => {
        try {
            const res = await DelectProfilePictureAPI(useId);
            return res;
        } catch (err: unknown) {
            const error = err as AxiosError<{ message?: string }>;
            throw new Error(error.response?.data.message || error.message);
...
const DashBoardSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(GetProFileDashAction.fulfilled, (state, action: PayloadAction<GetProfileRequest>) => {
                state.loading = false;
