...
const UploadFileComponents = () => {
    ...
    const { upload } = useAppSelector((state) => state.authentication);
    ...
    const onFileDrop = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (newFile) {
            ...
            if (fileSelected === null) {
                ...
                const upDatedList = [...fileList, { name: shortenedName, size: sizeFile }];
    ...
    const searchSubject = useSelector((state: RootState) => state.uploadFile.searchSubject) || []; //use selector hiển thị kết quả tìm kiếm môn học
    const searchFolder = useSelector((state: RootState) => state.uploadFile.searchFolder) || []; //use selector hiển thị kết quả tìm kiếm nội dung file

    // eslint-disable-next-line react-hooks/exhaustive-deps
    const debounceSearchFaculty = useCallback(
        debounce((value: string) => dispatch(SearchFacultyAction(value)).unwrap(), 1000),
        [dispatch, SearchFacultyAction],
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const debounceSearchSubject = useCallback(
        debounce((value: string) => dispatch(SearchSubjectAction(value)).unwrap(), 1000),
            ...
            <div className={cx('component-main-body')}>
                <div className={cx('main-body-top')}>
                    <div className={cx('body-top-list', { active: isActiveTitle(1) })}>
                        ...
                        <div className={cx('list-item-button', { active: isActiveBorder(1) })}></div>
                    </div>
                    <div className={cx('body-top-list', { active: isActiveTitle(2) })}>
                        ...
                        <div className={cx('list-item-button', { active: isActiveBorder(2) })}></div>
                    </div>
                    <div className={cx('body-top-list', { active: isActiveTitle(3) })}>
                        ...
                        <div className={cx('list-item-button', { active: isActiveBorder(3) })}></div>
                    </div>
                </div>
                {isDragging ? (
                        ...
                        {fileList.map((file: FileItem, index: number) => (
                            <div className={cx('center-detail-upload')} key={index}>
                                <div className={cx('detail-upload-header')}>
                                    ...
                                    <div className={cx('upload-header-icon')} onClick={handleDelectFileUpload}>
                                        <RestoreFromTrash />
                                    </div>
                                </div>
                    ...
                    <div className={cx('upload-succsess')}>
                        ...
                        <div className={cx('upload-success-information')}>
                            ...
                            <div className={cx('success-information-bottom')}>
                                <p>
                                    Tài liệu của bạn hiện đang được xử lý, quá trình này có thể mất vài phút. Hãy chú ý
                                    đến <span>chuông thông báo</span> để biết khi nào chúng đã sẵn sàng.
                                </p>
                            </div>
                        </div>
                        <div className={cx('upload-success-button')}>
                            <button className={cx('success-button-upload')} onClick={handleClickDefaultUpload}>
                                <div>
                                    <ArrowUpward />
                                </div>
                                Tải lên tài liệu khác
                            </button>
                            <button className={cx('success-button-back')} onClick={handleNavigateDocument}>
                ...
                {defaultUploadFile ? (
                    <div className={cx('main-detail-button')}>
                        <button className={cx('detail-button-comeback')} onClick={handleItemBack}>
                            <ArrowBack />
                            Quay lại
                        </button>
