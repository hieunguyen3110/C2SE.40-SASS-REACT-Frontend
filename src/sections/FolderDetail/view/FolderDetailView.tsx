...
const filterDocuments = (docs: any[] | undefined, searchValue: string) => {
    ...
    return docs.filter((doc) => {
        if (searchValue !== '') {
            return doc.title.toLowerCase().startsWith(searchValue.toLowerCase());
        } else {
            return doc;
        }
    });
        ...
        <div className={cx('folder-detail-view')}>
            ...
            {!isUpdating ? (
                <div className={cx('body')}>
                    ...
                    <div className={cx('docs-list')}>
                        ...
                        <div className={cx('list-body')}>
                            {filterDocuments(data.documents, filterDoc).length > 0 ? (
                                filterDocuments(data.documents, filterDoc).map((value: any, index: number) => (
                                    <div key={index} className={cx('doc')}>
                                        <a href={`/document/${value.docId}`} className={cx('doc-title')}>
                                            <DescriptionOutlinedIcon />
                                            {value.title}
                                        </a>
                                        <div className={cx('actions')}>
                ...
                <div className={cx('form-container')}>
                    {phase1Form}
                    <div className={cx('actions')}>
                        <button onClick={() => setIsUpdating(false)} style={{ backgroundColor: '#827878' }}>
                            Quay lại
                        </button>
                        <button onClick={handleUpdateFolder} style={{ backgroundColor: '#62d07a' }}>
                            Xác nhận chỉnh sửa
                        </button>
                    </div>
                </div>
