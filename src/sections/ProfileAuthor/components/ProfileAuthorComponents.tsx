...
import { updateFollowing } from '../../../redux/AuthenticationSlice/AuthenticationSlice';
...
const ProfilePersonalComponents = () => {
    ...
    const [shareCloseAnimation, setShareCloseAnimation] = useState<boolean>(false);
    ...
    const { viewProfile, isloading } = useAppSelector((state) => state.profileAuthor);
    const fullname = useAppSelector((state) => state.authentication.username);
    const [emailUser, setEmailUser] = useState('');
    const { follower, following } = useAppSelector((state) => state.authentication);
    ...
    const handleFollow = () => {
        if (viewProfile) {
            if (viewProfile.isFollow) {
                ...
                dispatch(updateFollowing(following - 1));
            } else {
                    ...
                    <div className={cx('author-component-information')}>
                        ...
                        <div className={cx('component-information-right')}>
                            ...
                            <div className={cx('information-right-item')}>
                                <div>
                                    <p>Chức vụ : {viewProfile.roles[0] === 'LECTURER' ? 'Giảng viên' : 'Sinh viên'}</p>
                                ...
                                >
                                    <div className={cx('bottom-share-title')}>
                                        <div className={cx('share-title-header')}>
                                            ...
                                            {/* <div> */}
                                            <img src={Delect} alt="Delect" onClick={() => handleCloseShare()} />
                                            {/* </div> */}
                                        </div>
                                        <div className={cx('share-title-content')}>
                                            <div className={cx('title-content-email')}>
                                                ...
                                                <button className={cx('content-email-button')}>Gửi Email</button>
                                            </div>
                                            <div className={cx('title-content-url')}>
                                                <h4>URL tài liệu</h4>
                                                <p>http://duytanforyou.com/vn/12345678/do-an-cdio-2024/...</p>
                                            </div>
                                            <div className={cx('title-content-copy')}>
                                                <button className={cx('button-coppy')}>Sao chép đường liên kết</button>
