    ...
    return (
        <div>
            <FormControl sx={{ mt: '20px', width: '100%' }}>
                <InputLabel id="demo-multiple-chip-label">Mã môn học</InputLabel>
                <Select
                    ...
                    value={courseName}
                    onChange={onChange}
                    input={<OutlinedInput id="select-multiple-chip" label="Mã môn học" />}
                ...
                >
                    {courses.map((value, index) => (
                        <MenuItem key={index} value={value.code} style={getStyles(value.name, courseName, theme)}>
