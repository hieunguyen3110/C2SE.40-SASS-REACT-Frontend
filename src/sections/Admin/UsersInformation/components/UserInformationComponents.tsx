...
const UserInformationComponents = () => {
    ...
    const { loading } = useAppSelector((state) => state.adminDashboard);
    // const { useId } = location.state || { useId: null };
    const { useId }: any = useParams();

    const handleOpenFileDialog = () => {
        if (useId) {
            dispatch(DelectProfilePictureAction(useId));
        } else {
            toast.info('Chưa lấy được id');
        }
    };
    useEffect(() => {
        if (useId !== null) {
    ...
    const { getProfileDash } = useSelector((state: RootState) => state.profileDashBoard);
                    ...
                    <div className={cx('profile-main-body')}>
                        <div className={cx('main-body-top')}>
                            <div className={cx('main-body-avatar')}>
                                <h3>Ảnh đại diện</h3>
                                <img src={getProfileDash?.profilePicture || avartar} alt="avatar" />
