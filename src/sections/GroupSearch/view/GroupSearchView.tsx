...
export default function GroupSearchView() {
    ...
    const filteredGroups = mockGroups.filter((group) => group.title.toLowerCase().includes(searchQuery.toLowerCase()));
    ...
    return (
        <Container className={cx('groupSearchView')}>
            ...
            <Typography className={cx('resultsCount')}>{filteredGroups.length} groups found</Typography>
            ...
            {noGroupsFound ? (
                <Box className={cx('emptyState')}>
                    ...
                    <Typography className={cx('emptyStateDescription')}>
                        Hiện tại chúng tôi thấy bạn chưa tham gia hoặc tạo nhóm học tập nào hết. Bạn có thể tạo một nhóm
