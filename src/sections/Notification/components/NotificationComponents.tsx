...
const NotificationComponents = () => {
    ...
    const [selectItem, setSelectItem] = useState(notificationList.map(() => false));
    ...
    const handleMoveToTrash = () => {
        handleBackMenu();
        const isSelected = selectItem.reduce((acc: number[], item, index: number) => {
        ...
        if (typeNotify === 'deleted') {
            ...
            dispatch(updateNumberOfNotificationsTrash(numberOfNotificationsDelete - isSelected.length));
        } else {
            Promise.all([
                dispatch(MoveNotifyToTrashdAction(getNotificationIds)),
                dispatch(updateNumberOfNotificationsTrash(numberOfNotificationsDelete + isSelected.length)),
                dispatch(updateNumberOfNotificationsAll(numberOfNotifications - isSelected.length)),
            ]);
        }
    ...
    const handleMoveToSaved = () => {
        ...
        dispatch(updateNotificationList(getNotificationIds));
        dispatch(updateNumberOfNotificationsSaved(numberOfNotificationsSaved + isSelected.length));
        dispatch(updateNumberOfNotificationsAll(numberOfNotifications - isSelected.length));
    };
    ...
    const handelPageChange = (_: React.ChangeEvent<unknown>, value: number) => {
        ...
        const isAllSelected =
            Array.isArray(selectItem[value]) && selectItem[value].every((item: boolean) => item === true);
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            if (menuDelete && menuRef.current && !menuRef.current.contains(event.target as Node)) {
                if (ignoreClick) {
        ...
        const timer = setTimeout(() => {
            if (notificationList.length > 0) {
                const unReadNotifications = notificationList.filter(
                    (notification) =>
                        !notification.isRead && !processedNotifications.current.has(notification.notificationId),
                );
                ...
                <input type="text" placeholder="Tìm kiếm thông báo..." />
                        ...
                        <div className={cx('body-list-item')}>
                            ...
                            <div className={cx('list-item-body')}>
                                    ...
                                    {menuDelete && (
                                        <div className={cx('body-menu-del')} ref={menuRef}>
                                            <ul>
                                                ...
                                                {typeNotify === 'all' && (
                                                    <li onClick={handleMoveToSaved}>Lưu lại thông báo này?</li>
                                                )}
                                ...
                                <div className={cx('item-body-bottom', opacityMenu && 'opacity-menu')}>
                                    {Loading ? (
                                        <Loader height={20} />
                                    ) : (
                                                ...
                                                <div className={cx('body-bottom-notication')} key={index}>
                                                    <div className={cx('notication-info')}>
                                                        <input
                                                            type="checkbox"
                                                            checked={
                                                                selectItem[(currentPage - 1) * itemsPerPage + index]
                                                            }
                                                            onChange={() =>
                                                        ...
                                                        />
                                                        <span className={cx(`${item.isRead && 'isRead'}`)}></span>
                                                        <img src={bookmark} />
                                                        <p>{item.message}</p>
                                                    </div>
                                                    <div className={cx('notication-time')}>
                                                        <span>{calculateTimeDifference(item.createdAt)}</span>
                                                    </div>
                                                </div>
                                            );
                                        })
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        Bạn có chắc là{' '}
                        {typeNotify === 'deleted' ? 'xoá thông báo này vĩnh viễn' : 'chuyển vào thùng rác'}?
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
